...
#' a list of helpers to prepare a selected violing plot with
#' mean and standard deviation under the normality assumption.
#'
...
#'
#' @returns A {ggplot2} object with wrapped violin plots and p-values.
#'
#' @export
draw_violins <- function(d0, df, fit, help, scl, rt_vars, which = "brains") {
  # Brain structures ----
  if (which == "brains") {
    ...
    d0 |>
      ...
      ggplot2::scale_colour_manual(values = c("navyblue","brown")) +
  ...
  } else if (which == "cognition_1" || which == "cognition_osa") {
    p <- fit |>
      mass() |>
      dplyr::mutate(
        p = paste0(zerolead(p.value), bh_adjust(p.value)), # p-value labels
  ...
  } else if (which == "cognition_2" || which == "cognition_pd") {
    p <- fit |>
      mass(type = "full") |>
